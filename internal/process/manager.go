...
func (m *Manager) getRealIP(rule *models.ForwardRule) {
	...
	// 构建代理 URL
	proxyURL := fmt.Sprintf("http://127.0.0.1:%d", rule.LocalPort)

	// 如果使用 socks5，需要使用 socks5 协议
	if rule.LocalType == "socks5" || rule.LocalType == "socks4" || rule.LocalType == "auto" {
		proxyURL = fmt.Sprintf("socks5://127.0.0.1:%d", rule.LocalPort)
	}

	// 创建 HTTP 客户端（简化版本，实际应该支持 socks5 代理）
	client := &http.Client{
		Timeout: 10 * time.Second,
	}

	// 尝试获取 IP（使用多个 IP 查询服务）
	ipServices := []string{
		"https://api.ipify.org",
