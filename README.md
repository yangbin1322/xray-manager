...
**故障转移与链式代理**
- 链式代理：多节点链式串联，流量依次经过各节点转发；只含 1 个节点时等同普通代理
- 故障转移：多节点主备切换，主节点故障时自动切换到备用节点
- 二者均支持测速、健康检测、流量监控，可作为系统代理或 TUN 目标
- 编辑运行中的节点 / 链式代理 / 故障转移后按新配置自动重启，修改立即生效
...
- **TUN 模式**：虚拟网卡接管所有程序的流量，转发到选中的节点，不遵循系统代理的程序也能走代理（与系统代理二选一；Windows 需管理员权限，未提权时可一键以管理员身份重启）
- **断网保护**：系统代理 / TUN 的目标意外失效时保持代理、不让流量走直连并提示；只有手动停止或删除目标才自动关闭代理
...
- **TUN 模式**：通过虚拟网卡接管所有程序的流量，转发到选中的节点 / 链式代理 /
  故障转移。不遵循系统代理的程序（游戏、命令行工具等）也能走代理。
  工具栏改为「系统代理 / TUN / 关闭代理」，两种模式二选一。
  Windows 下需要管理员权限：以普通权限运行时会提示并通过 UAC 以管理员身份重启，
  重启后自动恢复节点并开启 TUN。关闭程序即关闭 TUN，不跨重启保留
- **断网保护**：系统代理 / TUN 的目标意外失效（进程崩溃、启动后验证不通被停用、
  出口 IP 变化被停用等）时，代理保持开启、宁可断网也不让流量走直连，并提示你处理；
  目标恢复运行后自动恢复。只有**手动停止或删除**目标时才自动关闭代理。
  目标本地端口变更时代理自动改指向新端口；TUN 进程意外退出时自动重新拉起
- 编辑正在运行的节点、链式代理、故障转移后按新配置**自动重启**，修改立即生效。
  普通节点只在连接参数变化时重启（只改别名、备注不打断连接），
  运行中也可以直接修改本地端口
- 链式代理支持只包含 **1 个节点**，此时等同普通代理

**修复**

- 修复不先做健康检测、直接批量启动整个订阅时，几乎所有节点都被判为「代理连接失败」
  并自动停用。原因是批量验证中每停用一个不通节点就重启一次整片进程，切断了同片其他
  节点进行中的验证，好节点也跟着被判不通，连锁停掉整片。现在验证期间停用不通节点只作
  标记，等整批验证结束后统一重建一次；探测途中进程被重启时换新进程重新探测
- 关闭程序时先关闭系统代理 / TUN 再停止节点，停止节点期间不再断网
